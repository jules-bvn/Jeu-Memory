...
// ============================================================================
// 2. SYSTÈME DE RANKED : MATHÉMATIQUES ELO (Basé sur les Erreurs)
// ============================================================================
...
/**
 * Calcule la variation d'Elo en pénalisant fortement le nombre d'erreurs
 */
function calculerVariationElo(difficulte, temps, erreurs, indices = 0) {
  let baseGain = 0;
  let tempsMax, erreursMax;

  // Barème d'exigences
  if (difficulte === 'easy') { baseGain = 15; tempsMax = 25; erreursMax = 6; }
  else if (difficulte === 'medium') { baseGain = 25; tempsMax = 50; erreursMax = 12; }
  else { baseGain = 35; tempsMax = 80; erreursMax = 18; } 

  // Bonus de temps
  let bonusTemps = (tempsMax - temps) * 0.5; 
  // Calcul basé sur les erreurs (Très punitif)
  let bonusErreurs = (erreursMax - erreurs) * 2.5; 

  let variation = Math.round(baseGain + bonusTemps + bonusErreurs);

  // Malus tactique
  variation -= (indices * 10);
...
const inputJoueur2 = document.getElementById('nomJoueur2');
const inputYoutube = document.getElementById('lienYoutube'); // Lien YT

// Toggle Joueur 2
selectModeJeu.addEventListener('change', (e) => {
  if (e.target.value === '1vs1') {
    zoneJoueur2.classList.remove('hidden');
    inputJoueur2.required = true;
  } else {
    zoneJoueur2.classList.add('hidden');
    inputJoueur2.required = false;
  }
});
...
formulaireJeu.addEventListener('submit', async function (evenement) {
  ...
  const pseudo2 = inputJoueur2.value; 
  ...
  const lienYt = inputYoutube.value; 
  ...
  // Lancement du fond sonore Youtube
  gestionnaireDOM.lancerMusiqueYoutube(lienYt);

  try {
    const donnees = await ApiService.creerPartie(pseudo, difficulte);
    jeu.demarrerPartie(donnees.id, pseudo, difficulte, nomCollection, imagesPersonnalisees, mode, pseudo2);
  } catch (erreur) {
    jeu.demarrerPartie(Math.floor(Math.random() * 10000), pseudo, difficulte, nomCollection, imagesPersonnalisees, mode, pseudo2);
  } finally {
...
document.getElementById('boutonSabotage').addEventListener('click', () => {
  jeu.utiliserSabotage();
});
...
jeu.auFini = async (resultat) => {
  ...
  // SCÉNARIO 2 : Fin de Duel 1vs1
  if (resultat.mode === '1vs1') {
    let gagnant = "";
    if (resultat.scoreJ1 > resultat.scoreJ2) gagnant = `🏆 Victoire de ${resultat.nomJoueur} !`;
    else if (resultat.scoreJ2 > resultat.scoreJ1) gagnant = `🏆 Victoire de ${resultat.nomJoueur2} !`;
    else gagnant = "🤝 Match Nul !";

    alert(`⚔️ Fin du Duel !\n\n${gagnant}\n\nScores finaux :\n🟢 ${resultat.nomJoueur} : ${resultat.scoreJ1} paires\n🔴 ${resultat.nomJoueur2} : ${resultat.scoreJ2} paires`);
    gestionnaireDOM.afficherFormulaire();
    return; 
  }

  // SCÉNARIO 3 : Victoire Solo Fun (Normal, Fast, Chaos)
  if (resultat.mode !== 'classe') {
    let msgIndices = resultat.indices > 0 ? `\n🔍 Indices utilisés : ${resultat.indices}` : "";
    alert(`🎉 Victoire ${resultat.nomJoueur} !\n\n⏱️ Temps final : ${resultat.temps}s\n❌ Erreurs totales : ${resultat.erreurs}${msgIndices}\n\n(Mode Fun : Aucun impact sur ton Elo)`);
    gestionnaireDOM.afficherFormulaire();
  } 
  
  // SCÉNARIO 4 : Victoire Mode Classé
  else {
    let variation = calculerVariationElo(resultat.difficulte, resultat.temps, resultat.erreurs, resultat.indices);
    eloActuel += variation;
    if (eloActuel < 500) eloActuel = 500; 
    ...
    scores.forEach((score, index) => {
      ...
      html += `
        <tr>
          ...
          <td style="padding: 0.75rem; border-bottom: 1px solid rgba(255,255,255,0.1); color: #f87171;">${score.erreurs || 0}</td>
        </tr>
      `;
    });
  }
